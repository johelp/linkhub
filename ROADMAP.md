...
- Hooks de código para programa de afiliados (Rewardful, comisión recurrente de por vida vía Stripe): script gateado por `NEXT_PUBLIC_REWARDFUL_API_KEY` (sin la env var, cero cambio de comportamiento) en `layout.tsx`; `UpgradeButton.tsx` manda el `Rewardful.referral` a `/api/checkout`, que lo adjunta a la metadata del customer de Stripe (no al `client_reference_id`, que ya identifica al usuario de LinkHub para el webhook). Falta la cuenta de Rewardful en sí — ver `SETUP.md § Programa de afiliados`, es alta manual en su panel, no hay nada más que programar. **Pendiente de decisión**: el usuario pidió comparar alternativas gratuitas (Endorsely, Tapfiliate) o un sistema propio antes de confirmar Rewardful — ver conversación, los hooks de Rewardful quedan armados pero podrían cambiarse de proveedor sin tocar el resto del flujo (el patrón — script + referral a metadata de Stripe — es parecido entre herramientas)
